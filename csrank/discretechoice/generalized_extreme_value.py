...
class GeneralizedNestedLogit(DiscreteObjectChooser, Tunable):
    ...
    def __init__(self, n_features, n_objects, loss_function='', n_tune=500, n_sample=500, alpha=1e-3, random_state=None,
        ...
        self.nests_ind = np.array(list(combinations(np.arange(n_objects), 2)))
        self.n_nests = len(self.nests_ind)
        self.alpha = alpha
        self.random_state = check_random_state(random_state)
        self.logger = logging.getLogger(GeneralizedNestedLogit.__name__)
