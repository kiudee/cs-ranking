...
class FATELinearCore(Learner):
    ...
    def _construct_model_(self, n_objects):
        self.X = tf.placeholder("float64", [None, n_objects, self.n_object_features])
        self.Y = tf.placeholder("float64", [None, n_objects])
        std = 1 / np.sqrt(self.n_object_features)
        self.b1 = tf.Variable(self.random_state.normal(0, std, self.n_hidden_set_units))
        self.W1 = tf.Variable(self.random_state.normal(0, std, (self.n_object_features, self.n_hidden_set_units)))
        self.W2 = tf.Variable(self.random_state.normal(0, std, (self.n_object_features + self.n_hidden_set_units)))
        self.b2 = tf.Variable(self.random_state.normal(0, std, 1))
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epochs):
                for start in range(0, n_instances, self.batch_size):
                    ...
                    sess.run(self.optimizer, feed_dict={self.X: X[start:end], self.Y: Y[start:end]})
                    if verbose == 1:
                        progress_bar(end, n_instances, status='Fitting')
                if (epoch + 1) % 100 == 0:
                    c = sess.run(self.loss, feed_dict={self.X: X, self.Y: Y})
                    self.logger.info("Epoch {}: cost {} ".format((epoch + 1), np.mean(c)))
                self.step_decay(epoch)
            training_cost = sess.run(self.loss, feed_dict={self.X: X, self.Y: Y})
            self.logger.info("Fitting completed {} epochs done with loss {}".format(epochs, training_cost.mean()))
            self.weight1 = sess.run(self.W1)
            self.bias1 = sess.run(self.b1)
            self.weight2 = sess.run(self.W2)
            self.bias2 = sess.run(self.b2)
