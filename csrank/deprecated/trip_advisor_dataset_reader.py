...
class TripAdviosrDataset(object):
    def __init__(self, **kwargs):
        ...
        self.all_dataframes = pd.concat(list(self.datasets_dictionaries.values()), ignore_index=True)
    ...
    def data_for_city(self, city, features_to_remove=[]):
        ...
        if city not in self.city_names:
            raise Exception(
                'Data is not present for the city, hotel_dataset is present only for: ' + repr(self.city_names))
    ...
    def data_for_cities(self, cities=[BERLIN], features_to_remove=[]):
        ...
        norm_ranks = []
        for city in cities:
            headers, features, actual_ranks, normalized_ranks = self.data_for_city(city, features_to_remove)
