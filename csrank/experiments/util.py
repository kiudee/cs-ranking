...
__all__ = [
    ...
    "create_optimizer_parameters2",
    "get_scores",
    "datasets",
    "cfs",
    "ranking_metrics",
    "discrete_choice_metrics",
    "choice_metrics",
    "lp_metric_dict",
...
datasets = {
    SYNTHETIC_OR: ObjectRankingDatasetGenerator,
    DEPTH: DepthDatasetReader,
    SUSHI: SushiObjectRankingDatasetReader,
    IMAGE_DATASET: ImageDatasetReader,
    TAG_GENOME_OR: TagGenomeObjectRankingDatasetReader,
    SENTENCE_ORDERING: SentenceOrderingDatasetReader,
    LETOR_OR: LetorListwiseObjectRankingDatasetReader,
    SYNTHETIC_DC: DiscreteChoiceDatasetGenerator,
    LETOR_DC: LetorListwiseDiscreteChoiceDatasetReader,
    MNIST_DC: MNISTDiscreteChoiceDatasetReader,
    LETOR_RANKING_DC: LetorRankingDiscreteChoiceDatasetReader,
    SUSHI_DC: SushiDiscreteChoiceDatasetReader,
    TAG_GENOME_DC: TagGenomeDiscreteChoiceDatasetReader,
    SYNTHETIC_CHOICE: ChoiceDatasetGenerator,
    MNIST_CHOICE: MNISTChoiceDatasetReader,
    LETOR_CHOICE: LetorRankingChoiceDatasetReader,
    EXP_CHOICE: ExpediaChoiceDatasetReader,
    EXP_DC: ExpediaDiscreteChoiceDatasetReader,
}

cfs = {
    FETA_CHOICE: FETAChoiceFunction,
    FATE_CHOICE: FATEChoiceFunction,
    GLM_CHOICE: GeneralizedLinearModel,
    RANKNET_CHOICE: RankNetChoiceFunction,
    CMPNET_CHOICE: CmpNetChoiceFunction,
...
ors = {
    ...
    LISTNET: ListNet,
    FATELINEAR_RANKER: FATELinearObjectRanker,
    FETALINEAR_RANKER: FETALinearObjectRanker,
    RANDOM_RANKER: RandomBaselineRanker,
}
dcms = {
    FETA_DC: FETADiscreteChoiceFunction,
    ...
    NLM: NestedLogitModel,
    GEV: GeneralizedNestedLogitModel,
    PCL: PairedCombinatorialLogit,
    RANKSVM_DC: PairwiseSVMDiscreteChoiceFunction,
    MLM: MixedLogitModel,
...
ranking_metrics = {
    "KendallsTau": kendalls_tau_for_scores_np,
...
callbacks_dictionary = {
    "EarlyStoppingWithWeights": EarlyStoppingWithWeights,
    "LRScheduler": LRScheduler,
    "DebugOutput": DebugOutput,
    "CheckConvergence": pm.callbacks.CheckParametersConvergence,
...
def log_test_train_data(X_train, X_test, logger):
    if isinstance(X_train, dict) and isinstance(X_test, dict):
        ...
        logger.info(
            "instances {} objects {} features {}".format(
                n_instances, n_objects, n_features
            )
        )
        logger.info(
            "Using Test Set dictionary of rankings with lengths {}".format(
                X_test.keys()
            )
        )
        logger.info(
            "Using Training Set dictionary of rankings with lengths {}".format(
                X_train.keys()
            )
        )
    if not isinstance(X_test, dict):
        ...
        logger.info(
            "Test Set instances {} objects {} features {}".format(n_i, n_o, n_f)
        )
    if not isinstance(X_train, dict):
        n_instances, n_objects, n_features = X_train.shape
        logger.info(
            "Train Set instances {} objects {} features {}".format(
                n_instances, n_objects, n_features
            )
        )
    return n_objects
