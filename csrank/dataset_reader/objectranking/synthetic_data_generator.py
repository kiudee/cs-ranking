...
from csrank.util import scores_to_rankings, create_pairwise_prob_matrix, quicksort
...
class SyntheticDatasetGenerator(DatasetReader):
    ...
    def splitter(self, iter):
        for i in iter:
            ...
            X_test, Y_test = self.dataset_function(**self.kwargs,
                                                   n_instances=self.n_test_instances, seed=10 * i + 32)
    ...
    def get_single_train_test_split(self):
        ...
        self.X, self.rankings = X_train, Y_train = self.dataset_function(**self.kwargs,
                                                                         n_instances=self.n_train_instances, seed=seed)
        self.__check_dataset_validity__()

        seed = self.random_state.randint(2 ** 32, dtype='uint32')
        self.X, self.rankings = X_test, Y_test = self.dataset_function(**self.kwargs, n_instances=self.n_test_instances,
                                                                       seed=seed)
        self.__check_dataset_validity__()
    ...
    def make_gp_transitive(self, n_instances=1000, n_objects=5, noise=0.0, n_features=100, kernel_params=None
                           , seed=42, **kwd):
    ...
    def make_gp_non_transitive(self, n_instances=1000, n_objects=5, n_features=100, center_box=(-10.0, 10.0),
        ...
        for inst in range(n_instances):
            ...
            ordering = np.array([np.where(obj == ranking)[0][0] for obj in objects])
            X.append(feature)
