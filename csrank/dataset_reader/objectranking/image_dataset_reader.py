...
class ImageDatasetReader(DatasetReader):
    def __init__(
        ...
        **kwargs,
        ...
        self.similarity_matrix_train_file = os.path.join(
            self.dirname, "train_similarity_matrix.csv"
        )
        self.labels_train_files = glob.glob(
            os.path.join(self.dirname, TRAIN_LABEL, "*.txt")
        )
        self.labels_train_files.sort()

        self.test_file = os.path.join(self.dirname, "val.hd5")
        self.similarity_matrix_test_file = os.path.join(
            self.dirname, "val_similarity_matrix.csv"
        )
        self.labels_test_files = glob.glob(
            os.path.join(self.dirname, VAL_LABEL, "*.txt")
        )
        self.labels_test_files.sort()
        ...
        if not (
            ...
            and os.path.isfile(self.similarity_matrix_train_file)
        ):
    ...
    def splitter(self, iter):
        for i in iter:
            self.X, self.Y = X_train, Y_train = self.make_similarity_based_dataset(
                datatype="train", seed=10 * i + 32
            )
            self.__check_dataset_validity__()

            self.X, self.Y = X_test, Y_test = self.make_similarity_based_dataset(
                datatype="test", seed=10 * i + 32
            )
            self.__check_dataset_validity__()
    ...
    def get_single_train_test_split(self):
        ...
        self.X, self.Y = X_train, Y_train = self.make_similarity_based_dataset(
            datatype="train", seed=seed
        )
        self.__check_dataset_validity__()

        self.X, self.Y = X_test, Y_test = self.make_similarity_based_dataset(
            datatype="test", seed=seed + 1
        )
        self.__check_dataset_validity__()
    ...
    def make_similarity_based_dataset(self, datatype="train", seed=42):
        ...
        for i in range(n_instances):
            subset = random_state.choice(
                image_features.shape[0], size=self.n_objects, replace=False
            )
    ...
    def create_multilabel_dataset(self, datatype="train"):
        ...
        label_vectors_dictionary = collections.OrderedDict(
            sorted(label_vectors_dictionary.items())
        )
        label_vectors = []
        ...
        for i, j in combinations_list:
            similarity_matrix_lin_list[
                get_key_for_indices(i, j)
        ...
        assert (
            self.n_features == image_features.shape[1]
        ), "Number of features not correct in dataset {}".format(
