...
class DatasetReader(metaclass=ABCMeta):
    def __init__(self, dataset_folder="", learning_problem=OBJECT_RANKING, **kwargs):
        ...
        if "pc2" in os.environ["HOME"]:
            dirname = os.path.join(
                os.environ["PFS_FOLDER"], "cs-ranking", "csrank", "datasets"
            )
        else:
            dirname = os.path.join(
                os.environ["HOME"], "cs-ranking", "csrank", "datasets"
            )
    ...
    def __check_dataset_validity__(self):
        if self.X is not None or self.Y is not None:
            if (
                self.learning_problem == OBJECT_RANKING
                or self.learning_problem == DYAD_RANKING
            ):
                assert len(self.X.shape) == 3, EXCEPTION_OBJECT_ARRAY_SHAPE.format(
                    self.learning_problem, self.X.shape
                )
                n_instances, n_objects, n_features = self.X.shape
                assert (
                    n_instances == self.Y.shape[0]
                ), EXCEPTION_OUTPUT_FEATURES_INSTANCES.format(
                    self.learning_problem, self.Y.shape[0], n_instances
                )
                assert (
                    n_objects == self.Y.shape[1]
                ), EXCEPTION_RANKINGS_FEATURES_NO_OF_OBJECTS.format(
                    self.Y.shape[1], n_objects
                )
                if self.learning_problem == OBJECT_RANKING:
                    assert self.Xc is None, EXCEPTION_UNWANTED_CONTEXT_FEATURES.format(
                        self.learning_problem
                    )

            if self.learning_problem == LABEL_RANKING:
                assert len(self.X.shape) == 2, EXCEPTION_CONTEXT_ARRAY_SHAPE.format(
                    self.learning_problem, self.X.shape
                )
            ...
            if self.learning_problem == DYAD_RANKING:
                assert len(self.Xc.shape) == 2, EXCEPTION_CONTEXT_ARRAY_SHAPE.format(
                    self.learning_problem, self.Xc.shape
                )
